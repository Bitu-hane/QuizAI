  ...
  const fetchReportData = async () => {
    ...
    try {
      ...
      try {
        ...
        students = res.data;
        console.log('✅ /admin/users returned:', students.length, 'users');
      } catch (e) {
        console.warn('⚠️ /admin/users failed, trying /users');
        const res = await API.get('/users');
        students = res.data;
        console.log('✅ /users returned:', students.length, 'users');
      }
      // Filter only students (if role field exists)
      const allStudents = students.filter((u: any) => u.role === 'student' || u.roles?.includes('student'));
      ...
      try {
        ...
        quizzes = res.data;
        console.log('✅ /admin/quizzes returned:', quizzes.length, 'quizzes');
      } catch (e) {
        console.warn('⚠️ /admin/quizzes failed, trying /quizzes');
        const res = await API.get('/quizzes');
        quizzes = res.data;
        console.log('✅ /quizzes returned:', quizzes.length, 'quizzes');
      }

      // ----- 3. FETCH RESULTS (ALL STUDENTS) -----
      let results: any[] = [];
      try {
        const res = await API.get('/admin/results');
        results = res.data;
        console.log('✅ /admin/results returned:', results.length, 'results');
      } catch (e) {
        console.warn('⚠️ /admin/results failed, trying /results');
        const res = await API.get('/results');
        results = res.data;
        console.log('✅ /results returned:', results.length, 'results');
      }

      // ----- 4. FETCH PROGRESS (ALL STUDENTS) -----
      let progressData: any[] = [];
      try {
        const res = await API.get('/admin/progress');
        progressData = res.data;
        console.log('✅ /admin/progress returned:', progressData.length, 'progress records');
      } catch (e) {
        console.warn('⚠️ /admin/progress failed, trying /progress');
        const res = await API.get('/progress');
        progressData = res.data;
        console.log('✅ /progress returned:', progressData.length, 'progress records');
      }
      ...
      try {
        ...
        subjects = res.data;
        console.log('✅ /admin/subjects returned:', subjects.length, 'subjects');
      } catch (e) {
        console.warn('⚠️ /admin/subjects failed, trying /subjects');
        const res = await API.get('/subjects');
        subjects = res.data;
        console.log('✅ /subjects returned:', subjects.length, 'subjects');
      }

      // ----- 6. FETCH LESSONS (for mapping) -----
      let lessons: any[] = [];
      try {
        const res = await API.get('/admin/lessons');
        lessons = res.data;
        console.log('✅ /admin/lessons returned:', lessons.length, 'lessons');
      } catch (e) {
        console.warn('⚠️ /admin/lessons failed, trying /lessons');
        const res = await API.get('/lessons');
        lessons = res.data;
        console.log('✅ /lessons returned:', lessons.length, 'lessons');
      }
      ...
      results.forEach((r: any) => {
        const percentage = r.totalQuestions > 0 ? (r.score / r.totalQuestions) * 100 : 0;
        totalScore += percentage;
        if (percentage >= 70) passedCount++;
      });
      ...
      // ----- SUBJECT PERFORMANCE (with highest/lowest) -----
      const subjectMap: { [key: number]: { name: string; totalScore: number; totalAttempts: number; passed: number; scores: number[] } } = {};
      ...
      // For each progress record, update subject stats
      progressData.forEach((p: any) => {
        if (subjectMap[p.subjectId]) {
          const high = p.highScore || 0;
          const taken = p.quizTaken || 0;
          const passed = p.quizPassed || 0;
          subjectMap[p.subjectId].totalScore += high;
          subjectMap[p.subjectId].totalAttempts += taken;
          subjectMap[p.subjectId].passed += passed;
          // store score for highest/lowest
          if (high > 0) subjectMap[p.subjectId].scores.push(high);
        }
      });

      const perf = Object.values(subjectMap).map((s) => {
        const avg = s.totalAttempts > 0 ? Math.round(s.totalScore / s.totalAttempts) : 0;
        const passRate = s.totalAttempts > 0 ? Math.round((s.passed / s.totalAttempts) * 100) : 0;
      ...
      allStudents.forEach((s: any) => {
        studentMap[s._id] = `${s.FName || ''} ${s.LName || ''}`.trim() || 'Student';
      });

      // Build quiz map
      const quizMap: { [key: number]: string } = {};
      quizzes.forEach((q: any) => {
        quizMap[q.quizId] = q.title || `Quiz ${q.quizId}`;
      });

      const recent = results
        .sort((a, b) => new Date(b.takeTime || b.createdAt).getTime() - new Date(a.takeTime || a.createdAt).getTime())
        .slice(0, 10)
        .map((r: any) => {
          const percentage = r.totalQuestions > 0 ? Math.round((r.score / r.totalQuestions) * 100) : 0;
          return {
            id: r._id,
            student: studentMap[r.studentId] || 'Student',
            quiz: quizMap[r.quizId] || `Quiz ${r.quizId}`,
            score: percentage,
            date: new Date(r.takeTime || r.createdAt).toLocaleDateString(),
            status: percentage >= 70 ? 'Passed' : 'Failed' as 'Passed' | 'Failed',
          };
        });
      ...
      console.log('✅ Admin report data loaded successfully');
          ...
          <TableContainer component={Paper} sx={{ borderRadius: 2 }}>
            <Table>
              ...
              <TableBody>
                {subjectPerformance.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} align="center">No subject data available</TableCell>
                  </TableRow>
                ) : (
                  subjectPerformance.map((subject) => (
                    <TableRow key={subject.subject} hover>
                      ...
                      <TableCell align="center">{subject.highestScore}%</TableCell>
                      <TableCell align="center">{subject.lowestScore}%</TableCell>
                      <TableCell align="center">
                        <Chip
                          label={`${subject.passRate}%`}
                          size="small"
                ...
                {recentActivity.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={5} align="center">
                      {error ? 'Error loading data' : 'No recent activity'}
                    </TableCell>
                  </TableRow>
                ) : (
                  recentActivity.map((activity) => (
